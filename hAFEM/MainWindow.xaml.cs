...
namespace hAFEM
{
  ...
  public partial class MainWindow
  {
    ...
    {
      _inputData = new InputData(FixMinus(MuTxbx.Text), FixMinus(BetaTxbx.Text), FixMinus(SigmaTxbx.Text), FixMinus(FTxbx.Text), double.Parse(AlphaTxbx.Text), double.Parse(EtaTxbx.Text)) { N = int.Parse(NTxbx.Text) };
    ...
    public void CalculateError()
    {
      ...
      {
        // h^3 / mu
        var multiplier1 = Math.Pow(points[i].H, 3) / _inputData.Mu(points[i].Middle);

        // f - beta * q` - sigma * q
        var multiplier2 = Math.Pow(
          _inputData.F(points[i].Middle)
          - _inputData.Beta(points[i].Middle)
            * points[i].MiddleResultDeriv
          - _inputData.Sigma(points[i].Middle)
            * points[i].MiddleResult,
          2.0);

        // 10 + Pe * Sh
        var multiplier3 = 1.0
          / (10
            + Pe(points[i].H, points[i].Middle)
              * Sh(points[i].H, points[i].Middle));

        var errorNormSquareI = multiplier1 * multiplier2 * multiplier3;

        points[i].ErrorNorm = Math.Sqrt(5.0 / 6.0 * errorNormSquareI);

        errorNormSquare += errorNormSquareI;

        uNormSquare += points[i].H * Math.Pow(points[i].MiddleResultDeriv, 2);
      }
      ...
      var errorCoef = Math.Sqrt(n / (errorNormSquare + uNormSquare)) * 100;

      for (var i = 0; i < n; i++)
      {
        points[i].Error = points[i].ErrorNorm * errorCoef;
      }
    }
    ...
    private double[,] InitializeMatrix()
    {
      ...
      for (int i = 1; i < n; ++i)
      {
        ...
        matrix[i, i - 1] = (_inputData.Mu(points[i - 1].Middle) / points[i - 1].H)
          * (-1
            - Pe(points[i - 1].H, points[i - 1].Middle) / 2.0
            + Sh(points[i - 1].H, points[i - 1].Middle) / 6.0);

        matrix[i, i] = _inputData.Mu(points[i - 1].Middle) / points[i - 1].H
          * (1
            + Pe(points[i - 1].H, points[i - 1].Middle) / 2.0
            + Sh(points[i - 1].H, points[i - 1].Middle) / 3.0)

          + _inputData.Mu(points[i].Middle) / points[i].H
          * (1
            - Pe(points[i].H, points[i].Middle) / 2.0
            + Sh(points[i].H, points[i].Middle) / 3.0);

        matrix[i, i + 1] = _inputData.Mu(points[i].Middle) / points[i].H
          * (-1
            + Pe(points[i].H, points[i].Middle) / 2.0
            + Sh(points[i].H, points[i].Middle) / 6.0);

        // My Formuls
        /*matrix[i, i - 1] = (_inputData.Mu(points[i - 1].Middle) / points[i - 1].H)
          * (-1 + (1.0 / 6)
          * Pe(points[i - 1].H, points[i - 1].Middle)
          * (Sh(points[i - 1].H, points[i - 1].Middle) - 3));

        matrix[i, i] = -1.0 * ((_inputData.Mu(points[i - 1].Middle) / points[i - 1].H)
          * (-1 + (1.0 / 6)
          * Pe(points[i - 1].H, points[i - 1].Middle)
          * (2 * Sh(points[i - 1].H, points[i - 1].Middle) + 3))
          
          + (_inputData.Mu(points[i].Middle) / points[i].H) 
          * (-1 + (1.0 / 6) * Pe(points[i].H, points[i].Middle) 
          * (2 * Sh(points[i].H, points[i].Middle) - 3)));

        matrix[i, i + 1] = (_inputData.Mu(points[i].Middle) / points[i].H) 
          * (-1 + (1.0 / 6) 
          * Pe(points[i].H, points[i].Middle) 
          * (Sh(points[i].H, points[i].Middle) + 3));*/
      }

      // Olesia's formuls
      matrix[n, n - 1] = _inputData.Mu(points[n - 1].Middle) / points[n - 1].H
        * (-1
          - Pe(points[n - 1].H, points[n - 1].Middle) / 2.0
          + Sh(points[n - 1].H, points[n - 1].Middle) / 6.0);

      matrix[n, n] = _inputData.Mu(points[n - 1].Middle) / points[n - 1].H
        * (1
          + Pe(points[n - 1].H, points[n - 1].Middle) / 2.0
          + Sh(points[n - 1].H, points[n - 1].Middle) / 3.0)
        + _inputData.Alpha;

      // My formuls
      /*matrix[n, n - 1] = (_inputData.Mu(points[n - 1].Middle) / points[n - 1].H) 
        * (-1 + (1.0 / 6) 
        * Pe(points[n - 1].H, points[n - 1].Middle) 
        * (2 * Sh(points[n - 1].H, points[n - 1].Middle) - 3));
      matrix[n, n] = (_inputData.Mu(points[n - 1].Middle) / points[n - 1].H) 
        * (-1 + (1.0 / 6) * Pe(points[n - 1].H, points[n - 1].Middle) 
        * (2 * Sh(points[n - 1].H, points[n - 1].Middle) + 3)) 
        + _inputData.Alpha;*/
    ...
    private static string FixMinus(string function)
    {
      return function[0] == '-' ? "0" + function : function;
    }
  }
}
